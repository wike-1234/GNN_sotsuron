...
# タイトルと軸ラベル
ax.set_title("GNN Linear Operator Visualization - Regression", fontsize=14)
ax.set_ylabel(f"Output Feature Index (Node:{target_output_node+1})", fontsize=12)
ax.set_xlabel(f"Input Feature Index (Node:{target_input_node+1})", fontsize=12)

# X軸ラベル: 特徴量(チャンネル)のインデックス
step = 10 
all_ticks = np.arange(K)
all_labels = [f"Ch {k+1}" for k in range(K)]
ax.set_xticks(all_ticks[::step]+0.5)
ax.set_xticklabels(all_labels[::step], rotation=45)

# Y軸ラベル: 出力ステップ番号 (数が多い場合は間引く)
step = 10 
all_ticks = np.arange(K)
all_labels = [f"Step {k}" for k in range(K)]
ax.set_yticks(all_ticks[::step]+0.5)
ax.set_yticklabels(all_labels[::step], rotation=45)
